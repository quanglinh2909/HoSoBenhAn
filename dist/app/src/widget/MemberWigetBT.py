...
class MemberWigetBT(QMainWindow, Ui_Form):
    def __init__(self, parent=None, context=None):
        super(MemberWigetBT, self).__init__(context)
    ...
    def search(self):
        ...
        if key == "":
            self.loadData()
            return
        self.list = self.memberService.search(key, 1, 100, NGOAI_TRU)
    ...
    def loadData(self, list=None):
        ...
        if list is None:
            self.list = self.memberService.getPage(self.page, 100, NGOAI_TRU)
            self.total = self.memberService.getTotal(NGOAI_TRU)
        else:
            self.list = list
            self.total = len(list)
